...
def long_tracks(db, min_length)
  results = db.execute("SELECT name FROM tracks WHERE milliseconds > #{min_length * 60_000} ORDER BY milliseconds ASC").flatten
  # p results
...
def albums_per_artist(db)
  results = db.execute("SELECT name FROM artists, JOIN ")
  p results
  # you'll need to JOIN two tables, as well as using GROUP BY and ORDER BY!
